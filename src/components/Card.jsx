    ...
    <Link to={`/pokemon/${pokemon.id}`} relative='route'>
      <article className=' relative my-4 p-10 m-3 shadow-lg rounded-lg border border-gray-300 transition hover:scale-105'>
        ...
        <div className=' absolute top-0 left-0 flex items-center justify-around w-full py-3 '>
          <h3 className='text-2xl font-bold text-white capitalize lg:text-4xl'>{pokemon.name}</h3>
          <p className='text-gray-200 font-medium lg:text-2xl'>#{pokemon.id}</p>
        </div>
        ...
        <p className=' absolute -bottom-3 translate-x-2/4 right-[50%] bg-yellow-300 rounded-full py-1 px-2 text-xs font-medium shadow-md xl:scale-110'>Go to details</p>
      </article>
    </Link>
